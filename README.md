...
The regular grammar used in this laboratory corresponds to Variant 26 and is defined as the tuple:
...
This grammar generates strings that always begin with the symbol d, and the derivation continues until only terminal symbols remain.
...
---
## Example Derivation
To better understand how the grammar generates strings, consider the following derivation.
...
This example illustrates how nonterminal symbols are gradually replaced using production rules until only terminal symbols remain, producing a valid word of the language.
...
---
## Example of Automaton Execution

To demonstrate how the finite automaton verifies strings, consider the input string:

dad

The automaton processes the string symbol by symbol.
...
Since the automaton finishes in the final state F, the string is accepted as part of the language.
...
The program follows several main steps to demonstrate the equivalence between regular grammars and finite automata:

1. Define the grammar by specifying its terminals, nonterminals, start symbol, and production rules.

2. Generate valid strings by repeatedly applying grammar production rules.

3. Convert the grammar into an equivalent finite automaton.

4. Simulate the automaton to test whether given strings belong to the language.

5. Display the results of generated strings and membership tests.

This algorithm shows how formal language theory concepts can be implemented programmatically.
